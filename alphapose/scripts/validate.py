...
sys.path.insert(1, '/content/drive/MyDrive/pose-estimation-research/alphapose')
...
from transforms import (flip, flip_heatmap,
...
def validate(m, heatmap_to_coord, batch_size=20, num_workers=2):
    ...
    if opt.ppose_nms:
        ...
        with open(cfg.DATASET.TEST.DET_FILE, 'w+') as fid:
            json.dump(kpt_json, fid)

    sysout = sys.stdout
    res = evaluate_mAP(cfg.DATASET.TEST.DET_FILE, ann_type='keypoints', ann_file=os.path.join(cfg.DATASET.TEST.ROOT, cfg.DATASET.TEST.ANN), halpe=halpe)
    sys.stdout = sysout
    return res


def validate_gt(m, cfg, heatmap_to_coord, batch_size=20, num_workers=2):
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        gt_AP = validate_gt(m, cfg, heatmap_to_coord, opt.batch, 2)
        detbox_AP = validate(m, heatmap_to_coord, opt.batch, 2)
    print('#####  gt: {} \n det box: {} mAP #####'.format(gt_AP, detbox_AP))